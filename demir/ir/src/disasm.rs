...
pub fn print_ir(nodes: &[IrNode], first_node_id: &IrNodeId) {
    ...
    while let Some(node_id) = node_stack.pop() {
        ...
        match node {
            ...
            IrNode::ExternalFunction { ty, params, name } => {
                println!("%{node_id} = ExternalFunction %{ty} \"{name}\"");
            ...
            IrNode::Function {
                ...
                println!("%{node_id} = Function %{ty} \"{name}\"");
            ...
            IrNode::Decoration { dst, kind } => {
                println!("%{node_id} = Decoration %{dst} %{kind}");
            },
        }
    }
}
